...
2.参见：Func_Test\Treeview_Horizontal_Scrolbar.py
使用Frame等方式重新编排所有控件，以解决问题1，2


#todo:
1. 设置最大数据搜索数量和显示数量
2. treeview某行被选中，输出选中的内容，如双击显示所有内容。
3. 导出所有数据
...
# ScrollBar vetical
scrb_V_Display = tkinter.Scrollbar(master, orient='vertical')
scrb_V_Display.grid(row=6, column=0, rowspan = DISPLAY_ROWS, padx=0, pady = 10, sticky=N+S+E)
# ScrollBar horizontal
scrb_H_Display = tkinter.Scrollbar(master, orient='horizontal')
scrb_H_Display.grid(row=6+DISPLAY_ROWS, column=0, rowspan = 2, columnspan=3, padx=10, pady = 1, sticky=N+S+E+W)
# Treeview to display
COLS = 20
columnRange = ()
...
tv_Display = ttk.Treeview(master, show='headings', yscrollcommand= scrb_V_Display.set, xscrollcommand=scrb_H_Display.set)
tv_Display['columns'] = columnRange
tv_Display['height'] = DISPLAY_ROWS
tv_Display.grid(row=6, column=1, rowspan = DISPLAY_ROWS, columnspan = 10, padx=0, pady = 10, sticky=W+E+N+S)
# Set column name and width
for i in range(1,COLS):
